(ns beerpressure.db.beer-test
  (:require [clojure.test :refer :all]
            [beerpressure.handler :refer :all]
            [beerpressure.db.common-test :refer :all]))

(use-fixtures :once db-setup-with-logged-user-fixture)

(deftest test-resolve-beer
  (testing "beer(id: 1) - 1 brewery"
    (let [graphql-query (long-str "{"
                                  "  beer(id: 1) {"
                                  "    id"
                                  "    name"
                                  "    description"
                                  "    ibu"
                                  "    alcoholPercent"
                                  "    imagePath"
                                  "    breweries {"
                                  "      id"
                                  "      name"
                                  "      description"
                                  "      imagePath"
                                  "      tags {"
                                  "        id"
                                  "        name"
                                  "      }"
                                  "      rating"
                                  "    }"
                                  "    tags {"
                                  "      id"
                                  "      name"
                                  "    }"
                                  "    rating"
                                  "    style {"
                                  "      id"
                                  "      name"
                                  "    }"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beer\": {"
                                      "      \"id\": 1,"
                                      "      \"name\": \"Molson Canadian\","
                                      "      \"description\": \"Une bière avec une feuille d'érable dessus...\","
                                      "      \"ibu\": 15,"
                                      "      \"alcoholPercent\": 5.0,"
                                      "      \"imagePath\": \"https:\\/\\/decrescente.net\\/images\\/suppliers\\/millercoors\\/molson\\/molson-canadian\\/canadian-bottle-lg.png\","
                                      "      \"breweries\": ["
                                      "        {"
                                      "          \"id\": 1,"
                                      "          \"name\": \"MolsonCoors\","
                                      "          \"description\": \"Ils ne font de la marde comme bière.\","
                                      "          \"imagePath\": \"https:\\/\\/images.radio-canada.ca\\/q_auto,w_1250\\/v1\\/ici-info\\/16x9\\/molson-montreal-notre-dame.jpg\","
                                      "          \"tags\": ["
                                      "            {"
                                      "              \"id\": 1,"
                                      "              \"name\": \"Industrielle\""
                                      "            }"
                                      "          ],"
                                      "          \"rating\": 1.5"
                                      "        }"
                                      "      ],"
                                      "      \"tags\": ["
                                      "        {"
                                      "          \"id\": 1,"
                                      "          \"name\": \"Industrielle\""
                                      "        },"
                                      "        {"
                                      "          \"id\": 4,"
                                      "          \"name\": \"Mauvais choix\""
                                      "        }"
                                      "      ],"
                                      "      \"rating\": 1.5,"
                                      "      \"style\": {"
                                      "        \"id\": 1,"
                                      "        \"name\": \"Light lager\""
                                      "      }"
                                      "    }"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beer(id: 5) - 2 breweries"
    (let [graphql-query (long-str "{"
                                  "  beer(id: 5) {"
                                  "    id"
                                  "    breweries {"
                                  "      id"
                                  "    }"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beer\": {"
                                      "      \"id\": 5,"
                                      "      \"breweries\": ["
                                      "        {"
                                      "          \"id\": 5"
                                      "        },"
                                      "        {"
                                      "          \"id\": 6"
                                      "        }"
                                      "      ]"
                                      "    }"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response)))))

(deftest test-resolve-beers
  (testing "beers(skip: 0, first: 1, orderBy: NAME, orderType: ASC)"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 1, orderBy: NAME, orderType: ASC) {"
                                  "    id"
                                  "    name"
                                  "    description"
                                  "    ibu"
                                  "    alcoholPercent"
                                  "    imagePath"
                                  "    breweries {"
                                  "      id"
                                  "      name"
                                  "      description"
                                  "      imagePath"
                                  "      tags {"
                                  "        id"
                                  "        name"
                                  "      }"
                                  "      rating"
                                  "    }"
                                  "    tags {"
                                  "      id"
                                  "      name"
                                  "    }"
                                  "    rating"
                                  "    style {"
                                  "      id"
                                  "      name"
                                  "    }"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 2,"
                                      "        \"name\": \"Blue\","
                                      "        \"description\": \"Une bière bleu... Étrange\","
                                      "        \"ibu\": 12,"
                                      "        \"alcoholPercent\": 5.0,"
                                      "        \"imagePath\": \"https:\\/\\/www.google.com\\/url?sa=i&source=images&cd=&cad=rja&uact=8&ved=2ahUKEwir8pfW-7DbAhWr5oMKHX3oDtMQjRx6BAgBEAU&url=https%3A%2F%2Fwww.liquormarts.ca%2Fproduct%2Flabatt-blue%2F24-x-355-ml&psig=AOvVaw1n1N32iTLqUZu3QGlvQy7F&ust=1527891003844616\","
                                      "        \"breweries\": ["
                                      "          {"
                                      "            \"id\": 2,"
                                      "            \"name\": \"Labatt\","
                                      "            \"description\": \"Une copie de MolsonCoors en pire...\","
                                      "            \"imagePath\": \"https:\\/\\/upload.wikimedia.org\\/wikipedia\\/fr\\/4\\/40\\/Labatt.png\","
                                      "            \"tags\": ["
                                      "              {"
                                      "                \"id\": 1,"
                                      "                \"name\": \"Industrielle\""
                                      "              }"
                                      "            ],"
                                      "          \"rating\": 1.1666666666666667"
                                      "          }"
                                      "        ],"
                                      "        \"tags\": ["
                                      "          {"
                                      "            \"id\": 1,"
                                      "            \"name\": \"Industrielle\""
                                      "          },"
                                      "          {"
                                      "            \"id\": 4,"
                                      "            \"name\": \"Mauvais choix\""
                                      "          }"
                                      "        ],"
                                      "        \"rating\": 0.0",
                                      "        \"style\": {"
                                      "          \"id\": 1,"
                                      "          \"name\": \"Light lager\""
                                      "        }"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 1, orderBy: NAME, orderType: DESC)"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 1, orderBy: NAME, orderType: DESC) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 4"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 1, orderBy: NAME, orderType: ASC, breweries: [1])"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 1, orderBy: NAME, orderType: ASC, breweries: [1]) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 1"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 1, orderBy: NAME, orderType: DESC, breweries: [2])"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 1, orderBy: NAME, orderType: DESC, breweries: [2]) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 2"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 2, orderBy: NAME, orderType: ASC, tags: [1])"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 2, orderBy: NAME, orderType: ASC, tags: [1]) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 2"
                                      "      },"
                                      "      {"
                                      "        \"id\": 1"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 2, orderBy: NAME, orderType: DESC, tags: [2])"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 2, orderBy: NAME, orderType: DESC, tags: [2]) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 4"
                                      "      },"
                                      "      {"
                                      "        \"id\": 5"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 2, orderBy: NAME, orderType: ASC, breweries: [1,3], tags: [2])"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 2, orderBy: NAME, orderType: ASC, breweries: [1,3], tags: [2]) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 3"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 2, orderBy: NAME, orderType: DESC, breweries: [1,3,4], tags: [2])"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 2, orderBy: NAME, orderType: DESC, breweries: [1,3,4], tags: [2]) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 4"
                                      "      },"
                                      "      {"
                                      "        \"id\": 3"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 2, orderBy: RATING, orderType: ASC)"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 2, orderBy: RATING, orderType: ASC) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 2"
                                      "      },"
                                      "      {"
                                      "        \"id\": 1"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 2, orderBy: RATING, orderType: DESC)"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 2, orderBy: RATING, orderType: DESC) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 4"
                                      "      },"
                                      "      {"
                                      "        \"id\": 5"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 2, orderBy: RATING, orderType: ASC, breweries: [1,2])"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 2, orderBy: RATING, orderType: ASC, breweries: [1,2]) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 2"
                                      "      },"
                                      "      {"
                                      "        \"id\": 1"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 2, orderBy: RATING, orderType: DESC, breweries: [2,3])"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 2, orderBy: RATING, orderType: DESC, breweries: [2,3]) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 3"
                                      "      },"
                                      "      {"
                                      "        \"id\": 2"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 2, orderBy: RATING, orderType: ASC, tags: [1])"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 2, orderBy: RATING, orderType: ASC, tags: [1]) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 2"
                                      "      },"
                                      "      {"
                                      "        \"id\": 1"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 2, orderBy: RATING, orderType: DESC, tags: [2])"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 2, orderBy: RATING, orderType: DESC, tags: [2]) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 4"
                                      "      },"
                                      "      {"
                                      "        \"id\": 5"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 2, orderBy: RATING, orderType: ASC, breweries: [1,3,5], tags: [2,3])"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 2, orderBy: RATING, orderType: ASC, breweries: [1,3,5], tags: [2,3]) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 3"
                                      "      },"
                                      "      {"
                                      "        \"id\": 5"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 2, orderBy: RATING, orderType: DESC, breweries: [1,3,4], tags: [1,2])"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 2, orderBy: RATING, orderType: DESC, breweries: [1,3,4], tags: [1,2]) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 4"
                                      "      },"
                                      "      {"
                                      "        \"id\": 3"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response))))
  (testing "beers(skip: 0, first: 10, orderBy: RATING, orderType: DESC, styles: [1,2])"
    (let [graphql-query (long-str "{"
                                  "  beers(skip: 0, first: 10, orderBy: RATING, orderType: DESC, styles: [1, 2]) {"
                                  "    id"
                                  "  }"
                                  "}")
          expected-response (long-str "{"
                                      "  \"data\": {"
                                      "    \"beers\": ["
                                      "      {"
                                      "        \"id\": 3"
                                      "      },"
                                      "      {"
                                      "        \"id\": 1"
                                      "      },"
                                      "      {"
                                      "        \"id\": 2"
                                      "      }"
                                      "    ]"
                                      "  }"
                                      "}")
          response (execute-graphql-query graphql-query)]
      (is (is-data-equal response expected-response)))))

(deftest test-beer-mutations
  (testing "beer mutations"
    (let [insert-graphql-query (long-str "mutation insertBeer {"
                                         "  insertBeer(name: \\\"A name\\\", description: \\\"A description\\\", ibu: 1, alcoholPercent: 2.5, imagePath: \\\"A path\\\", breweries: [1, 2], tags: [\\\"Industrielle\\\", \\\"A new one\\\"], style: 1) {"
                                         "    id"
                                         "    name"
                                         "    description"
                                         "    ibu"
                                         "    alcoholPercent"
                                         "    imagePath"
                                         "    breweries {"
                                         "      id"
                                         "      name"
                                         "      description"
                                         "      imagePath"
                                         "      tags {"
                                         "        id"
                                         "        name"
                                         "      }"
                                         "      rating"
                                         "    }"
                                         "    tags {"
                                         "      id"
                                         "      name"
                                         "    }"
                                         "    rating"
                                         "    style {"
                                         "      id"
                                         "      name"
                                         "    }"
                                         "  }"
                                         "}")
          update-graphql-query (long-str "mutation updateBeer {"
                                         "  updateBeer(id: 100, name: \\\"A new name\\\", description: \\\"A new description\\\", ibu: 2, alcoholPercent: 3.5, imagePath: \\\"A new path\\\", breweries: [2], tags: [\\\"Industrielle\\\", \\\"A new one 2\\\"], style: 2) {"
                                         "    id"
                                         "    name"
                                         "    description"
                                         "    ibu"
                                         "    alcoholPercent"
                                         "    imagePath"
                                         "    breweries {"
                                         "      id"
                                         "      name"
                                         "      description"
                                         "      imagePath"
                                         "      tags {"
                                         "        id"
                                         "        name"
                                         "      }"
                                         "      rating"
                                         "    }"
                                         "    tags {"
                                         "      id"
                                         "      name"
                                         "    }"
                                         "    rating"
                                         "    style {"
                                         "      id"
                                         "      name"
                                         "    }"
                                         "  }"
                                         "}")
          delete-graphql-query (long-str "mutation deleteBeer {"
                                         "  deleteBeer(id: 100)"
                                         "}")
          expected-insert-response (long-str "{"
                                             "  \"data\": {"
                                             "    \"insertBeer\": {"
                                             "      \"id\": 100,"
                                             "      \"name\": \"A name\","
                                             "      \"description\": \"A description\","
                                             "      \"ibu\": 1,"
                                             "      \"alcoholPercent\": 2.5,"
                                             "      \"imagePath\": \"A path\","
                                             "      \"breweries\": ["
                                             "        {"
                                             "          \"id\": 1,"
                                             "          \"name\": \"MolsonCoors\","
                                             "          \"description\": \"Ils ne font de la marde comme bière.\","
                                             "          \"imagePath\": \"https:\\/\\/images.radio-canada.ca\\/q_auto,w_1250\\/v1\\/ici-info\\/16x9\\/molson-montreal-notre-dame.jpg\","
                                             "          \"tags\": ["
                                             "            {"
                                             "              \"id\": 1,"
                                             "              \"name\": \"Industrielle\""
                                             "            }"
                                             "          ],"
                                             "          \"rating\": 1.5"
                                             "        },"
                                             "        {"
                                             "          \"id\": 2,"
                                             "          \"name\": \"Labatt\","
                                             "          \"description\": \"Une copie de MolsonCoors en pire...\","
                                             "          \"imagePath\": \"https:\\/\\/upload.wikimedia.org\\/wikipedia\\/fr\\/4\\/40\\/Labatt.png\","
                                             "          \"tags\": ["
                                             "            {"
                                             "              \"id\": 1,"
                                             "              \"name\": \"Industrielle\""
                                             "            }"
                                             "          ],"
                                             "        \"rating\": 1.1666666666666667"
                                             "        }"
                                             "      ],"
                                             "      \"tags\": ["
                                             "        {"
                                             "          \"id\": 1,"
                                             "          \"name\": \"Industrielle\""
                                             "        },"
                                             "        {"
                                             "          \"id\": 100,"
                                             "          \"name\": \"A new one\""
                                             "        }"
                                             "      ],"
                                             "      \"rating\": 0.0,"
                                             "      \"style\": {"
                                             "        \"id\": 1,"
                                             "        \"name\": \"Light lager\""
                                             "      }"
                                             "    }"
                                             "  }"
                                             "}")
          expected-update-response (long-str "{"
                                             "  \"data\": {"
                                             "    \"updateBeer\": {"
                                             "      \"id\": 100,"
                                             "      \"name\": \"A new name\","
                                             "      \"description\": \"A new description\","
                                             "      \"ibu\": 2,"
                                             "      \"alcoholPercent\": 3.5,"
                                             "      \"imagePath\": \"A new path\","
                                             "      \"breweries\": ["
                                             "        {"
                                             "          \"id\": 2,"
                                             "          \"name\": \"Labatt\","
                                             "          \"description\": \"Une copie de MolsonCoors en pire...\","
                                             "          \"imagePath\": \"https:\\/\\/upload.wikimedia.org\\/wikipedia\\/fr\\/4\\/40\\/Labatt.png\","
                                             "          \"tags\": ["
                                             "            {"
                                             "              \"id\": 1,"
                                             "              \"name\": \"Industrielle\""
                                             "            }"
                                             "          ],"
                                             "        \"rating\": 1.1666666666666667"
                                             "        }"
                                             "      ],"
                                             "      \"tags\": ["
                                             "        {"
                                             "          \"id\": 1,"
                                             "          \"name\": \"Industrielle\""
                                             "        },"
                                             "        {"
                                             "          \"id\": 101,"
                                             "          \"name\": \"A new one 2\""
                                             "        }"
                                             "      ],"
                                             "      \"rating\": 0.0,"
                                             "      \"style\": {"
                                             "        \"id\": 2,"
                                             "        \"name\": \"Belgian ale\""
                                             "      }"
                                             "    }"
                                             "  }"
                                             "}")
          verification-query "SELECT * FROM beer WHERE id_beer = 100"]
      (is (is-data-equal (execute-graphql-query insert-graphql-query) expected-insert-response))
      (is (not (empty? (query-sql-statement verification-query))))
      (is (is-data-equal (execute-graphql-query update-graphql-query) expected-update-response))
      (execute-graphql-query delete-graphql-query)
      (is (empty? (query-sql-statement verification-query))))))
