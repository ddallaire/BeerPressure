...
-- name: get-beer
SELECT id_beer AS id, name, description, ibu, alcohol_percent, image_path, rating, style FROM beer_with_rating WHERE id_beer = :id;


-- name: get-beers-ordered-by-name-asc
SELECT id_beer AS id, name, description, ibu, alcohol_percent, image_path, rating, style FROM beer_with_rating
    ORDER BY name ASC
...
SELECT beer.id_beer AS id, beer.name, description, ibu, alcohol_percent, image_path, rating, style FROM beer_with_rating AS beer
    INNER JOIN beer_brewery ON beer_brewery.id_beer = beer.id_beer
    WHERE id_brewery IN (:breweries)
    GROUP BY beer.id_beer, beer.name, description, ibu, alcohol_percent, image_path, rating, style
    ORDER BY name ASC
    LIMIT :first OFFSET :skip;
...
SELECT beer.id_beer AS id, name, description, ibu, alcohol_percent, image_path, rating, style FROM beer_with_rating AS beer
    INNER JOIN beer_brewery ON beer_brewery.id_beer = beer.id_beer
    WHERE id_brewery IN (:breweries)
    GROUP BY beer.id_beer, beer.name, description, ibu, alcohol_percent, image_path, rating, style
    ORDER BY rating DESC, name ASC
